...
def generate_launch_description():
    ...
    scan_remap = PythonExpression(["'/scan:=/' + '", ns, "' + '/scan'"])

    args = [
        DeclareLaunchArgument("ns", default_value="student01"),
        DeclareLaunchArgument(
            "bag_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("cartographer_parallel"), "bags"]
            ),
        ),
        DeclareLaunchArgument(
            "map_yaml_file",
    ...
    return LaunchDescription(args + [node, bag_play])
