...
__global__ void ScoreBlockCachedMapSharedKernel(
    ...
    float* score) {
  // TEST5 keeps TEST4's cached-grid/shared-memory kernel.
...
bool EnsureCachedGrid(const std::vector<unsigned char>& grid, const int w,
                      const int h, unsigned char** const device_grid) {
  // TEST4 map cache: upload each score_all grid level once, then reuse it.
  ...
  if (cudaMalloc(reinterpret_cast<void**>(&uploaded), bytes) !=
      cudaSuccess) {
    return false;
  }
...
void PruneCandidatesOutsideScanBounds(
  ...
  // TEST5: remove candidates whose shifted scan bounding box misses the grid.
...
bool score_all_gpu_block_cached_map_shared_pruned(
  ...
  if (!EnsureCachedGrid(grid, w, h, &d_grid)) return false;
  ...
  bool ok = CopyVectorToDevice(px, p, &d_px) &&
  ...
  ScoreBlockCachedMapSharedKernel<<<m, kBlockThreads>>>(
      d_grid, w, h, d_px, d_py, p, d_cx, d_cy, m, d_score);
