...
__global__ void ScoreThreadPerCandidateKernel(
    ...
    float* score) {
  // TEST2: one CUDA thread owns one candidate and loops over all scan points.
...
bool score_all_gpu_thread_per_candidate(
  ...
  ScoreThreadPerCandidateKernel<<<blocks, threads>>>(d_grid, w, h, d_px, d_py,
                                                     p, d_cx, d_cy, n,
                                                     d_score);
