...
__global__ void ScoreThreadCachedMapKernel(
    ...
    float* score) {
  // Isolated TEST2+TEST4: keep one-thread-per-candidate, cache map only.
...
bool score_all_gpu_thread_cached_map(
  ...
  ScoreThreadCachedMapKernel<<<blocks, threads>>>(d_grid, w, h, d_px, d_py, p,
                                                  d_cx, d_cy, n, d_score);
